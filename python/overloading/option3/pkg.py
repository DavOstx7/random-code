...
class OverloadManager:
    ...
    def find(self, group: str, *args, **kwargs) -> Function:
        if group not in self.managers_by_group:
            raise ValueError(f"No function has been added to group '{group}'")

        matches = self.managers_by_group[group].find_matches(*args, **kwargs)
        if len(matches) == 0:
            raise ValueError(f"The provided arguments for group '{group}' do not match any signature")
        elif len(matches) > 1:
            raise ValueError(f"The provided arguments for group '{group}' match more than a single signature")
        return matches[0]
...
def add(group, *functions: Callable):
    for f in functions:
        manager.add(group, Function(f))


def call(group: str, *args, **kwargs):
    function = manager.find(group, *args, **kwargs)
