...
int main(){
	...
	s.PushConstructor();
	s.PushConstructor();
	//now it's at 32, //+12 deallocation because of move operator
	Timer t;

	cout << "~~~Poping entity into another entity reference\n";
	Entity& x = s.Pop(); //this just uses the location of the array it was popped from (nothing happens).

	cout << "~~~Poping entity into another entity (no reference)\n"; 
	Entity x1 = s.Pop(); //copy constructor + 4

	cout << "~~~Creating empty entity\n";
	Entity z; //copy constructor + 4
	cout << "~~~Assigning a pop to the entity\n";
	z = s.Pop(); //copy operator 
